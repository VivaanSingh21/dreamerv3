...
class CarlaEnv(object):
    ...
    def __init__(self,
        ...
        self.client.set_timeout(60.0)  # P2: was 5.0, too short for load_world
    ...
    def _simulator_step(self, action, dt=0.05):
        ...
        # CARLA_REWARD selects the reward. Default 'sar' = the protocol-locked 0.9.6 reward
        # (unchanged, comparable to SAR Table 2). 'dbc' = the DBC 0.9.8 reward, verified against
        # facebookresearch/deep_bisim4control CARLA_0.9.8/.../carla_env.py 2026-08-26: dense
        # lane-centering divisor, binary collision at 1.0, steer/brake penalties at 0.1. Numbers
        # under 'dbc' are NOT comparable to SAR's 193 (different objective). See docs 17/18.
        _rew = os.environ.get('CARLA_REWARD', 'sar')
        if _rew in ('dbc', 'dbc_nobrake'):
            colliding = float(collision_intensities_during_last_time_step > 0.)
            brake_pen = 0.0 if _rew == 'dbc_nobrake' else 0.1 * brake  # dbc_nobrake: drop the brake penalty (2026-08-27 experiment)
            reward = vel_s * dt / (1. + dist_from_center) - 1.0 * colliding - brake_pen - 0.1 * abs(steer)
        else:
        ...
        info['dist_from_center'] = dist_from_center
        info['vel_s'] = vel_s
